...
pub async fn hash_password(password: String) -> Result<String> {
    ...
    //     Ok(
    //         jPasswordHash::generate(Argon2::default(), password, &salt_string)
    ...
    // .map_err(|e| ApiErrResp::internal_server_error(e.to_string()))??)
    let salt_string = SaltString::generate(&mut OsRng);
    let password = PasswordHash::generate(Argon2::default(), password, &salt_string)
        .map_err(|e| ApiErrResp::internal_server_error(e.to_string()))?
        .to_string();
...
pub async fn verify_password(password: String, hash: String) -> Result<()> {
    ...
    let hash =
        PasswordHash::new(&hash).map_err(|e| ApiErrResp::internal_server_error(e.to_string()))?;
    hash.verify_password(&[&Argon2::default()], password)
        .map_err(|e| match e {
            argon2::password_hash::Error::Password => {
                ApiErrResp::unauthorized(Some("Invalid email or password".to_string()))
